...
void Lifetime(char* infile) {
   ...
   TFile f(TString::Format("%s.lifetime_qaqc_out.root",infile),"recreate");
  ...
  Double_t QA,QC;
  Double_t lifetime;
  Double_t QA_err, QC_err, lifetime_err,QA_QC_err, QA_QC;
  ...
  lifetime_err = ((TMath::Power(par[1],2))==0.) ? 0. : (TMath::Abs((1/(TMath::Power(par[1],2)))*par_error[1]));
  QA_QC_err = (QA + QA_err)/(QC - QC_err) - QA/QC;
  
  QA_QC = (QA!=QA || QC!=QC || QC==0) ? 0 : (QA/QC);
  
  std::cout << "***************Purity Information *******************" << endl;
  std::cout << "QC    : " << QC << " +/- " << QC_err << endl;
  std::cout << "QA    : " << QA << " +/- " << QA_err << endl;
  std::cout << "QA/QC : " << QA_QC << " +/- " << QA_QC_err << endl;
  //std::cout << "Electron Life : " << lifetime*1000<<" ms"<< " +/- " << lifetime_err*1000<<" ms"<< endl;
  
  std::ofstream ofs (TString::Format("%s.json",infile), std::ofstream::out);
  ofs << "{\n\"QC\": "<<QC<<",\n";
  ofs << "\"QC_err\": "<<QC_err<<",\n";
  ofs << "\"QA\": "<<QA<<",\n";
  ofs << "\"QA_err\": "<<QA_err<<",\n";
  ofs << "\"QAQC\": "<<QA_QC<<",\n";
  ofs << "\"QAQC_err\": "<<QA_QC_err<<"\n}";
  ofs.close();
  ...
  std::cout<<"Writing canvas and histogram..." << std::endl;
  qaqc->Write();
  qAqC->Write();
